...
export default function Hero({
  ...
  return (
    <section className="overflow-hidden pb-16 pt-10 lg:pt-14">
      <Container className="grid min-w-0 items-start gap-8 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)] lg:gap-12 xl:gap-16">
        <Reveal variant="fade-right" className="flex min-w-0 flex-col gap-5">
          <div className="flex flex-col gap-0">
            <p className="text-hero-mini text-white">{eyebrow}</p>
            <h1 className={`${titleClass} mb-2 text-secondary`}>{title}</h1>
            <p className="text-hero-mini text-white">{subtitle}</p>
          </div>

          <div className="flex flex-col gap-4 pt-2">
            {paragraphs.map((paragraph) => (
              <p key={paragraph} className="text-body max-w-xl text-white/75">
                {paragraph}
              </p>
            ))}
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4 pt-2">
            <img src="/google.webp" alt="Amazon" className="w-40" />
            <img src="/reviews.webp" alt="Amazon" className="w-40" />
          </div>
        ...
        <Reveal variant="fade-left" delay={120} className="min-w-0">
          <LeadForm {...form} variant="card" className="w-full min-w-0 lg:mt-2" />
        </Reveal>
      </Container>
    </section>
  );
